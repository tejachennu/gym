...
const firebaseConfig = {
  apiKey: "AIzaSyAdNnrno1B1leS-y-DwYeg18N-8fsegzAw",
  authDomain: "dynamic-linking-ec557.firebaseapp.com",
  projectId: "dynamic-linking-ec557",
  storageBucket: "dynamic-linking-ec557.firebasestorage.app",
  messagingSenderId: "424111550757",
  appId: "1:424111550757:web:bffcc84334ff27cafd2acc",
  measurementId: "G-93DVYV4YL8"
};
...
async function runFullSeeding() {
  ...
  let adminUser;
  try {
    const cred = await signInWithEmailAndPassword(auth, "admin@powerhouse.com", "Admin@123456");
    adminUser = cred.user;
    console.log("✅ Authenticated as Admin (admin@powerhouse.com)");
  } catch (err) {
    if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
      const cred = await createUserWithEmailAndPassword(auth, "admin@powerhouse.com", "Admin@123456");
      adminUser = cred.user;
      console.log("✅ Created Admin user (admin@powerhouse.com)");
    } else {
      throw err;
    }
  }

  // 2. Seed Admin User Doc
  ...
  console.log("✅ Admin user profile doc created in Firestore");

  // 3. Authenticate / Create Client
  let clientUser;
  try {
    const cred = await signInWithEmailAndPassword(auth, "client@powerhouse.com", "Client@123456");
    clientUser = cred.user;
    console.log("✅ Authenticated as Client (client@powerhouse.com)");
  } catch (err) {
    if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
      const cred = await createUserWithEmailAndPassword(auth, "client@powerhouse.com", "Client@123456");
      clientUser = cred.user;
      console.log("✅ Created Client user (client@powerhouse.com)");
    } else {
      console.error("Client auth warning:", err.message);
    }
  }
  ...
  if (clientUser) {
    await setDoc(doc(db, "Users", clientUser.uid), {
      uid: clientUser.uid,
      displayName: "John Fitness Client",
      email: "client@powerhouse.com",
      phone: "+1 555-0122",
      role: "client",
      status: "active",
      createdAt: new Date().toISOString()
    }, { merge: true });
    console.log("✅ Client user profile doc created in Firestore");
  }

  // Switch back to Admin for seeding data
  await signInWithEmailAndPassword(auth, "admin@powerhouse.com", "Admin@123456");
  ...
  if (plansSnap.empty) {
    for (const p of plansData) {
      await addDoc(plansRef, { ...p, createdAt: new Date().toISOString() });
    }
    console.log("✅ 6 Membership Plans seeded into Firestore");
  } else {
    console.log(`ℹ️ ${plansSnap.size} Membership Plans already exist in Firestore`);
  }
  ...
  if (dietSnap.empty) {
    await addDoc(dietRef, {
      clientId: clientUser ? clientUser.uid : "demo-client-uid",
    ...
    console.log("✅ Sample Diet Plan seeded into Firestore");
  } else {
    console.log(`ℹ️ Diet plans already exist in Firestore`);
  }
  ...
  if (workoutSnap.empty) {
    ...
    console.log("✅ Sample Workout Plan seeded into Firestore");
  } else {
    console.log(`ℹ️ Workout plans already exist in Firestore`);
  }
  ...
  if (bloodSnap.empty) {
    ...
    console.log("✅ Sample Blood Report seeded into Firestore");
  } else {
    console.log(`ℹ️ Blood reports already exist in Firestore`);
  }
  ...
  if (notifSnap.empty) {
    await addDoc(notifRef, {
      userId: clientUser ? clientUser.uid : "demo-client-uid",
    ...
    console.log("✅ Sample Notification seeded into Firestore");
  } else {
    console.log(`ℹ️ Notifications already exist in Firestore`);
  }

  console.log("\n🎉 ALL APIS & FIRESTORE COLLECTIONS VERIFIED AND SEEDED SUCCESSFULLY!");
  process.exit(0);
}

runFullSeeding().catch(err => {
  console.error("❌ Error running full seed:", err);
  process.exit(1);
});
